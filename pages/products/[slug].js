...
import { getAllProductsWithSlug, getProduct, getRelatedProducts } from '../../lib/bigcommerce-graphql-api'
import { getPages } from '../../lib/bigcommerce-rest-api'
...
export default function Product({ allPages, product, relatedProducts }) {
  ...
  const handleThumbnailImageClick = (e) => {
    document.getElementById('mainGalleryImage').src = e.target.getAttribute('data-image-gallery-new-image-url')
  }
          ...
          <>
            ...
            <article>
              <Head>
                ...
                <meta property="og:image" content={product.images.edges[0].url640wide} />
              </Head>
              <section className="font-sans flex flex-col lg:flex-row">
                <div className="flex flex-col w-full lg:w-1/2">
                  <div className="flex-grow">
                    <img alt="" src={product.images.edges[0].node.url1280wide} id="mainGalleryImage" />
                  </div>
                  <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-5 mb-5">
                    {product.images.edges.map((image, idx) => (
                        <img 
                          ...
                          data-image-gallery-new-image-url={image.node.url1280wide}
                ...
                <div className="w-full lg:w-1/2 flex flex-col p-8">
                  <h1 className="tracking-wide font-large text-2xl font-bold text-gray-800">{product.name}</h1>
                  <p className="uppercase tracking-wide text-sm font-bold text-gray-700">{product.brand?.name}</p>
                  <p className="text-3xl text-gray-900"><ProductPrices product={product} /></p>
            ...
            <div className="mb-24">
              ...
              {relatedProducts.length > 0 && <ProductGrid products={relatedProducts} />}
            </div>
          </>
        )}
      </Container>
